...
4. Complete the approved native acceptance in `docs/ACCEPTANCE.md`.
5. Create the `.mfp` with MudForge's native package creator. Do not zip the
   staging directory manually.
6. Verify the exported package with `tools/build_release.py --verify-package`.
7. Re-run the full offline suite and inspect the final Git diff before release.
