...
1. Confirm the disposable client is MudForge 1.2.2454 or newer. Install the
   repository library and both plugins; verify displayed version 0.4.2.
...
10. Create the atomic package through **Settings → Packages → Create Package**,
   ...
   with package id `com.samroberts.aardwolf-core`, version 0.4.2, and minimum
   client 1.2.2454.
11. Save the export as `dist/aardwolf-core-0.4.2.mfp`, then run:

   ```bash
   python3 tools/build_release.py --verify-package dist/aardwolf-core-0.4.2.mfp
   ```
