...
on("aardwolf.core.consumer.declare", function(payload)
  if payload.consumerId == "aardwolf-chat" then
    emit("aardwolf.core.consumer.registration", {
      consumerId = payload.consumerId, ok = true, protocol = 1, version = "0.4.0", apiVersion = "0.4.0",
    })
  end
end)
...
table.insert(TEST.loadedPlugins, { id = "aardwolf-core", name = "Aardwolf Core", version = "0.4.0", enabled = true })
emit("aardwolf.core.ready", { protocol = 1, version = "0.4.0" })
